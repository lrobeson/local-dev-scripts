...
cd ~/Dropbox/Local\ Dev\ Config/dot\ files && git add . && git commit -m "Automatic snapshot" && git push origin master &&
cd ../scripts && git add . && git commit -m "Automatic snapshot" && git push origin master